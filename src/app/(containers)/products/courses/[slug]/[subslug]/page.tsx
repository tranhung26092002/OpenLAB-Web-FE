...
export default function CourseView() {
  ...
  return (
    ...
    >
      <div
        className={`w-[24%]  py-4  h-screen bg-red-200 overflow-y-auto transition-opacity ease-out duration-200 ${
          isOpenMenu
          ...
          <ul>
            {indexItem.map((item, index) => {
              return (
                <li
                  key={index}
                  className="py-1 hover:bg-[#eee] rounded-sm pl-4"
                >
                  <Link href={`#${index}`}>{item.nameItem}</Link>
                </li>
              );
            })}
          </ul>
      ...
      <div
        className={` flex flex-col justify-center transition-all ease-out duration-200 items-center  ${
          isOpenMenu ? "w-[76%] px-4" : "w-full px-24 z-10"
        }`}
      >
